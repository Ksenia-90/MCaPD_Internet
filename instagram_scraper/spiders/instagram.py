...
class InstagramSpider(scrapy.Spider):
    ...
    def user_page_parse(self, response: HtmlResponse, username: str):
        ...
        if user_id is not None:
            ...
            main_person['pk'] = str(user_id)
            main_person['username'] = username
            yield main_person
    ...
    def parse_item(self, user_item, user_id, type_link):
        ...
        person['pk'] = str(user_item['pk'])
        person['username'] = user_item['username']
        person['full_name'] = user_item['full_name']
        person['profile_pic_url'] = user_item['profile_pic_url']
        if type_link == 'followers':
            person['subscribed_to'] = str(user_id)
        else:
            person['subscribed_from'] = str(user_id)
        return person
